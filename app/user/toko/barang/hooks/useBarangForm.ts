...
export default function useBarangForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Get CSRF token using the function from axios.ts
      const csrfToken = getCsrfToken();

      // First, create the product - this will generate a slug based on the name
      ...
      if (productResponse.data.status === "success") {
        ...
        // Then upload images using the product ID
        ...
        router.push(`/user/toko/barang/detail/${productSlug}`);
      }
    } catch (error: any) {
